...
class UserController extends Controller {
    ...
    public function performLogin(Request $request) {
        if (env('POLR_ACCT_CREATION_RECAPTCHA')) {
            ...
            // Log::info('captcha', ['req' => $request->input('captcha')]);
            if ($validator->fails()) {
                return redirect(route('login'))->withErrors($validator->messages());
            }
        }
        ...
        if ($credentials_valid != false) {
            ...
            return redirect(route('login'))->with('error', 'Invalid password or inactivated account. Try again.');
        }
    }
    ...
    public function performSignup(Request $request) {
        ...
        if (env('POLR_ACCT_CREATION_RECAPTCHA')) {
            ...
            if ($validator->fails()) {
                return redirect(route('signup'))->withErrors($validator->messages());
            }
